...
void Helper::loadConfig()
{
    ...
    _inactiveTitleBarColor =
        appGroup.readEntry("inactiveBackground",
        globalGroup.readEntry("inactiveBackground", palette.color(QPalette::Disabled, QPalette::Highlight)));
...
{
    QColor outline = KColorUtils::mix(palette.color(QPalette::Button), palette.color(QPalette::WindowText), 0);
...
{
    return KColorUtils::mix(focusColor(palette), palette.color(QPalette::WindowText), 0.0);
}

QColor Helper::hoverOutlineColor(const QPalette &palette) const
{
    return KColorUtils::mix(hoverColor(palette), palette.color(QPalette::WindowText), 0.0);
}

//____________________________________________________________________
QColor Helper::buttonFocusOutlineColor(const QPalette &palette) const
{
    return KColorUtils::mix(buttonFocusColor(palette), palette.color(QPalette::ButtonText), 0.0);
}

QColor Helper::buttonHoverOutlineColor(const QPalette &palette) const
{
    return KColorUtils::mix(buttonHoverColor(palette), palette.color(QPalette::ButtonText), 0.0);
}
...
//____________________________________________________________________
QColor Helper::sliderHandleColor(const QPalette &palette, bool mouseOver, bool hasFocus, qreal opacity, AnimationMode mode) const
{
                               ...
                               qreal penAnimation) const
{
    ...
    if (flat) {
        if (down && enabled) {
            ...
            bgBrush = hasNeutralHighlight ? alphaColor(neutralText(palette), highlightBackgroundAlpha) : palette.color(QPalette::Button);
            penBrush = hasNeutralHighlight ? neutralText(palette) : palette.color(QPalette::Button);
        ...
        if (down && enabled) {
            ...
            bgBrush = palette.color(QPalette::Button);
            penBrush = hasNeutralHighlight ? neutralText(palette) : frameOutlineColor(palette);
        }
    }
